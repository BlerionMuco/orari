...
- **Multi-resource — shipped (feature 4, public-booking milestone 1):** the engine serves multi-chair shops. `getUnionAvailability` unions every active resource's open slots — each resource's own hours/time-off/bookings, deduped by start instant (not a merged calendar) — and `createAnyBooking` assigns a concrete free resource server-side (idempotency-first, candidates ordered least-booked-that-date with stable-id tie-break, `23P01` → drop+retry next, `23505` → return existing, shared `insertConfirmedBooking` keeps the `booking_confirmed` outbox enqueue on both paths). **Per-resource service-menu capability** stays deferred (V1: any active resource does any active service) — the engine already takes candidate resources, so it's a localized add.
...
- The booking wizard UI (service → resource → time picker → guest form → confirmation) consumes `getAvailableSlots()` / `getUnionAvailability()` (via the availability route, `resourceId` optional) + `createBookingAction()` (which routes to `createBooking` for a specific resource or `createAnyBooking` for "any available").
